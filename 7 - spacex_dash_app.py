...
def get_pie_chart(entered_site):    
    if entered_site == 'ALL':
        ...
        filtered_df = spacex_df.set_index('Launch Site')        
        filtered_df = filtered_df.loc[[entered_site],:]
        filtered_df = filtered_df.reset_index()
        count_df = filtered_df['class'].value_counts().sort_index().to_dict()
        fig_df = pd.DataFrame({'Launch Site':[entered_site, entered_site],
                               'class': list(count_df.keys()), 'class_count': list(count_df.values())})
        fig = px.pie(fig_df, values='class_count', 
        names=list(count_df.keys()), 
